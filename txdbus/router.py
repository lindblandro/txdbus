...
class MessageRouter:
    ...
    def addMatch(self, callback, mtype=None, sender=None, interface=None,
        ...
        # print 'ROUTER ADDING MATCH: ', mtype, path, interface, member, 'arg',
        # arg
        ...
        # Simple
        if mtype:
            r.add('_messageType', mtype)
        if sender:
            r.add('sender', sender)
        if interface:
            r.add('interface', interface)
        if member:
            r.add('member', member)
        if path:
            r.add('path', path)
        if destination:
            r.add('destination', destination)

        # Complex
        if path_namespace:
            r.add('path_namespace', path_namespace)
        if args:
            r.add('args', args)
        if arg_paths:
            r.add('arg_paths', arg_paths)
        if arg0namespace:
            r.add('arg0namespace', arg0namespace)
